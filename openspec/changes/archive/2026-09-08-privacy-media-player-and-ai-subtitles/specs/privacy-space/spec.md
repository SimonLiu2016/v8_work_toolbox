...
The system SHALL maintain unlock status strictly in memory during an active session and automatically lock the Privacy Space upon inactivity or manual request.

#### Scenario: Manual quick lock
- **WHEN** user clicks the Lock button in the Privacy Space header
- **THEN** system immediately clears the in-memory unlock state and returns to the PIN lock screen

#### Scenario: Automatic lock on idle
- **WHEN** user remains inactive in Privacy Space for longer than the configured timeout (e.g. 5 minutes)
